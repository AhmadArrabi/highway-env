""""CHANGE WIDTH OF LANES AND FEATURES (DASHED LINE SPACING)"""
...
"""AHMAD: THIS FILE IS *KEY* CHANGE DISPLAY SETTINGS OF ENVIRONMENT COLORS AND ALL, SEARCH FOR COLOUR AND CHANGE"""
...
class LaneGraphics(object):
    ...
    def draw_stripes(cls, lane: AbstractLane, surface: WorldSurface,
        ...
        for k, _ in enumerate(starts):
            if ((abs(starts[k] - ends[k]) > 0.5 * cls.STRIPE_LENGTH) and not (lane.line_types == (LineType.STRIPED, LineType.STRIPED))):
                pygame.draw.line(surface, surface.NEON,
            ...
            if(lane.line_types == (LineType.STRIPED, LineType.STRIPED)):
                pygame.draw.line(surface, surface.NEON,
                                 (surface.vec2pix(lane.position(starts[k], lats[k]))),
                                 (surface.vec2pix(lane.position(ends[k], lats[k]))),
                                 max(surface.pix(cls.STRIPE_WIDTH_2), 1))
